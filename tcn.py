...
class TemporalConvNet(nn.Module):
    def __init__(self, num_inputs, num_channels, kernel_size=2, dropout=0.2, max_length=39, attention=True):
        """
        ...
        for i in range(num_levels):
            ...
            in_channels = num_inputs if i == 0 else num_channels[i - 1]  # 输入层的通道数为语料库中的词的数量，其它层通通道数都为1200 int
            out_channels = num_channels[i]  # 输出通道数都为1200 int
            if attention == True:
